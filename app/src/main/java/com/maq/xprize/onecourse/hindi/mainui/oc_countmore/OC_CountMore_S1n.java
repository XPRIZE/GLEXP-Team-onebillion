 ...
 */
public class OC_CountMore_S1n extends OC_SectionController
{
    ...
    private void checkChildDrag(OBGroup child, PointF pt) throws Exception
    {
        ...
        {
            gotItRight();
            child.setProperty("start_loc", OC_Generic.copyPoint(child.position()));
    ...
    {
        OC_CountMore_S1.hiliteClothes(child,true,this);
        if(child == objectDict.get(String.format("child_%s",eventAttributes.get("correct"))))
        {
            ...
            OC_CountMore_S1.hiliteClothes(child,false,this);
            waitForSecs(0.3f);
            if(!OBUtils.getBooleanValue(eventAttributes.get("tick")))
            {
                displayTick();
